...
public:
	...
	int Open(int nIdx,  UINT uiMode);
	int Close();
	int SensingLine(PBYTE pbTuchStatus);
	int TransToDevice(UINT uiSendID, BYTE byParam, LPVOID pOption, WORD wOptionLen, DWORD* pdwError);
...
private:
	...
	SOCKET ListenSocket;// ¼àÌýsocket
	SOCKET ClientSocket;// Á¬½Ósocket
