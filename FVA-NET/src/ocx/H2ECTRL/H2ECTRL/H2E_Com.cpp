...
int CH2ECom::Open(int nIdx, UINT uiMode)
{
	...
	char ServerPort[5];
	_itoa_s(nIdx, ServerPort, 5, 10);
	
	WSADATA wsaData;
	ListenSocket = INVALID_SOCKET;// 监听socket
...
int CH2ECom::SensingLine(PBYTE pbTuchStatus)
{
	...
	if(m_hCom){	
		...
		if(bReturn){
			if(dwModemStatus & MS_CTS_ON){//MS_RLSD_ON
				//TUCH_OUT1 active
				dwTStatus |= H2E_TOUCH_OUT1;
			}
			if(dwModemStatus & MS_DSR_ON){//MS_RING_ON
				//TUCH_OUT2 active
...
int CH2ECom::sendCommand(UINT uiSendID, DWORD dwSendLen, DWORD* pdwError)
{
	...
	iReturn = H2E_OK;
	//Send command data
	bytesSent = send(ClientSocket, // socket
				(char *)m_pSendBuffer, // 数据
				dwSendLen, // 数据长度
				0);// 标志
	if( bytesSent == SOCKET_ERROR)
	{
		//printf("\tCommunicationThread\tsend error %d\n", WSAGetLastError());
		iReturn = H2E_ERR_SOCKET_SEND;
...
int CH2ECom::recvResponse(UINT uiSendID, BYTE byOpt,  LPVOID* pSerData, WORD* pwSerLength, DWORD* pdwError)
{
	...
	int			i;
	int iResult = 0;
	BYTE *m_pRecvBuffer_tmp = m_pRecvBuffer;
	...
	*pdwError			= 0;
	...
	//check arrived respons in driver's buffer  H2E_RECVBUFF_MAX
	switch(uiSendID){
		/*
		...
		case H2E_COM_VER_1TON:
			...
			while(iResult == 0){
				Sleep(100);
				iResult = recv(ClientSocket, // socket
						(char *)m_pRecvBuffer, // 接收缓存
						H2E_RECVBUFF_MAX, // 缓存大小
						0);// 标志
			}
			break;
			*/
