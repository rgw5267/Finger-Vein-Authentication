...
//
#if FTDI_VCP
int CH2ECom::Open(int nIdx, UINT uiMode)
{

	int			iReturn;

	char cComPort[30] = { '\0' } ;	// 要打开的COM端口
	
	iReturn		= H2E_OK;

	sprintf_s( cComPort, "\\\\.\\COM%d", nIdx + 1 ) ;
	...
	// 打开 COM 端口
	HANDLE hCom = CreateFileA( cComPort, GENERIC_READ|GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL ) ;

	// 如果打开失败
	if ( hCom == INVALID_HANDLE_VALUE ) {
		iReturn = H2E_ERR_COMOPEN;//GetLastError();//(ErrCode: %d)
		return iReturn;
	}
	...
	//串口设置 （V1.10---2011/5/23）
	//H1E-USB兼容USB2.0/1.1通信，每个设置将被忽略。
	//begin
	DCB dcb;
	...
	//ReadFile()，WriteFile() 超时时间设置 (V1.10---2011/5/25)
	//请适当设置超时时间
	//begin
	COMMTIMEOUTS cts;
...
int CH2ECom::Open()
{
	...
	ftStatus= FT_OpenEx("FVA-USB",FT_OPEN_BY_DESCRIPTION, &ftHandle);
	...
	ftStatus = FT_SetDataCharacteristics(ftHandle, FT_BITS_8, FT_STOP_BITS_1, FT_PARITY_ODD);
	if(ftStatus != FT_OK){
		iReturn = -200;
		return iReturn;
	}
...
//
//Sensing touch sensor state (TUCH_OUT1, TUCH_OUT2) Method
//
...
int CH2ECom::SensingLine(PBYTE pbTuchStatus)
{
	int		iReturn;
	DWORD	dwModemStatus;
	BYTE	dwTStatus;
...
//
int CH2ECom::TransToDevice(UINT uiSendID, BYTE byParam, LPVOID pOption, WORD wOptionLen, DWORD* pdwError)
{
	int	iReturn;
	DWORD	dwSendLen;
...
int CH2ECom::sendCommand(UINT uiSendID, DWORD dwSendLen, DWORD* pdwError)
{
	...
	DWORD dwBytesInQue;
	FT_STATUS	ftStatus;
	FT_GetQueueStatus(ftHandle,&dwBytesInQue);
	if(dwBytesInQue > 0){
		//dummy read driver's buffer
		ftStatus = FT_Read(
			ftHandle,
...
#elif FTDI_D2XX
			while(dwBytesInQue == 0){
				Sleep(100);
				ftStatus = FT_GetQueueStatus(ftHandle, &dwBytesInQue);
			}
#endif
			break;
